...
DigitDisplay_t* Tm1637_Attach(gpio_num_t clk, gpio_num_t data, uint8_t brightness, uint8_t digit_count) {
    ...
    digitdisplay->digit_count = digit_count;
...
void Tm1637_DisplayAll(DigitDisplay_t* digitdisplay, uint8_t disp_data[]) {
    ...
    Tm1637_Coding_Full(digitdisplay, seg_data);
...
void Tm1637_DisplayBitAddPoint(DigitDisplay_t* digitdisplay, uint8_t bit_addr, uint8_t disp_data, uint8_t add_point) {
    ...
    seg_data = Tm1637_Coding_One(digitdisplay, disp_data, add_point);
    Tm1637_Start(digitdisplay);               // Start signal sent to TM1637 from MCU
...
void Tm1637_DisplayStr(DigitDisplay_t* digitdisplay, char str[], uint16_t loop_delay) {
		int end = strlen(str);
		if(end <= digitdisplay->digit_count) {
			for (int i = 0; i < digitdisplay->digit_count; i++) {
			...
			int offset=-digitdisplay->digit_count;

			for (int i = 0; i <= end+digitdisplay->digit_count; i++) {
				for (int j = offset, k=0; j < digitdisplay->digit_count+offset; j++,k++) {
					if (j<0 || j>=end) {
...
void Tm1637_ClearDisplay(DigitDisplay_t* digitdisplay) {
    uint8_t clear[digitdisplay->digit_count];
...
void Tm1637_Coding_Full(DigitDisplay_t* digitdisplay, uint8_t disp_data[]) {
    for (uint8_t i = 0; i < digitdisplay->digit_count; i++) {
        disp_data[i] = Tm1637_Coding_One(digitdisplay, disp_data[i], POINT_OFF);
    }
}
